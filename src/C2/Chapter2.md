...
其中一些通过在系统上运行代理（也称为导出器）来收集其统计信息来运行。这些代理要么执行操作系统可观察性工具（例如 iostat（1） 或 sar（1））并解析输出的文本（被认为效率低下），要么直接从操作系统库和内核接口中读取。监控产品支持一组自定义代理，用于从特定目标（Web 服务器、数据库和语言运行时）导出统计信息。
...
# #2.9.3 摘要-自启动
如果尚未执行监控，请检查操作系统是否至少提供了 summary-since-boot 值，这些值可用于与当前值进行比较。
